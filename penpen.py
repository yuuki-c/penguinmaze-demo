...
def draw_screen(): # ゲーム画面を描く
    ...
    draw_txt("STAGE "+str(stage), 200, 30, 30, "white")
    draw_txt("SCORE "+str(score), 520, 30, 30, "lime")
...
def move_enemy(): # ホワイトを動かす
    global idx, tmr, white_x, white_y, white_d, white_a
    speed = 10
    if white_x%60 == 30 and white_y%60 == 30:
        white_d = random.randint(0, 6)
        if white_d >= 4:
            if pen_y < white_y:
                white_d = DIR_UP
            if pen_y > white_y:
    ...
    if white_d == DIR_UP:
        if check_wall(white_x, white_y, white_d, speed) == False:
            white_y = white_y - speed
    if white_d == DIR_DOWN:
        if check_wall(white_x, white_y, white_d, speed) == False:
            white_y = white_y + speed
    if white_d == DIR_LEFT:
        if check_wall(white_x, white_y, white_d, speed) == False:
            white_x = white_x - speed
    if white_d == DIR_RIGHT:
        if check_wall(white_x, white_y, white_d, speed) == False:
            white_x = white_x + speed
    white_a = white_d*3 + ANIMATION[tmr%4]
    if abs(white_x-pen_x) <= 40 and abs(white_y-pen_y) <= 40:
        idx = 2
        tmr = 0


def move_enemy2(): # 熊を動かす
...
def main(): # メインループ
    ...
    if idx == 3: # ゲームオーバー
        draw_txt("GAME OVER", 360, 270, 40, "white")
        if tmr == 50:
            idx = 0
...
img_pen = [
    ...
    tkinter.PhotoImage(file="image_penpen/heart1.png")
]
img_white = [
    tkinter.PhotoImage(file="image_penpen/white00.png"),
    tkinter.PhotoImage(file="image_penpen/white01.png"),
    tkinter.PhotoImage(file="image_penpen/white02.png"),
    tkinter.PhotoImage(file="image_penpen/white03.png"),
    tkinter.PhotoImage(file="image_penpen/white04.png"),
    tkinter.PhotoImage(file="image_penpen/white05.png"),
    tkinter.PhotoImage(file="image_penpen/white06.png"),
    tkinter.PhotoImage(file="image_penpen/white07.png"),
    tkinter.PhotoImage(file="image_penpen/white08.png"),
    tkinter.PhotoImage(file="image_penpen/white09.png"),
    tkinter.PhotoImage(file="image_penpen/white10.png"),
    tkinter.PhotoImage(file="image_penpen/white11.png")
]
img_kuma = [
